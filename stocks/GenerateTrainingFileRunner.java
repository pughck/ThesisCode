...
public class GenerateTrainingFileRunner {
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i < args.length; i++) {
			...
			String outputFile = basePath + "stanford/training-" + totalMin + ".txt";

			GenerateFile gf = new GenerateFile(inputFile, outputFile, date, true, totalMin);
			try {
				gf.execute();
			} catch (IOException e) {
				e.printStackTrace();
			...
			outputFile = basePath + "open/training-" + totalMin + ".txt";

			gf = new GenerateFile(inputFile, outputFile, date, true, totalMin);
			try {
				gf.execute();
			} catch (IOException e) {
				e.printStackTrace();
			...
			outputFile = basePath + "of/training-" + totalMin + ".txt";
